...
//storing the amount of "classifications" in here
std::vector<int> classifications;
//to store current points
...
//functions to add classifications based on amount of points gathered by the laser
void classification(std::vector<float> ranges) {
	
	bool inClassifications = false;
	
	//get the total amount of points obtained from the laser
	currentPointsAmount = ranges.size();
	
	//Add first point if classifications vector is empty
	if(classifications.empty())
	{
		classifications.push_back(currentPointsAmount);
		//ROS_DEBUG_STREAM(classifications.size());
	}
	else//Possibly add the point in the vector
	{
		//iterate through the classifications vector
		for(int i = 0; i < classifications.size(); i++)
		{
			//if the current points amount has already been found, change bool paramater to true
			if(classifications.at(i) == currentPointsAmount)
			{
				inClassifications = true;
				break;
			}
		}
		//If points amount wasn't found, we have a new type of object, maybe
		if(inClassifications == false)
		{	
			//adding new type
			classifications.push_back(currentPointsAmount);
			//ROS_DEBUG_STREAM(classifications.size());
...
//    ROS_DEBUG_STREAM("Found " << angles.size() << " points");
	...
	if(point.x < 0.0 || point.x > 40.0 || point.y < 0.0 || point.y > 40.0)
		result1 = 1;
    }
...
void scanCallback( const sensor_msgs::LaserScan::ConstPtr& scan )
{
        ...
        it!=scan->ranges.end(); it++, angle_tmp=angle_tmp+inc )
    {
        ...
        if( *it > scan->range_min && *it < scan->range_max )
        {
		    ...
		    classification(contiguousRanges);
