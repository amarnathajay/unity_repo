...
// C/C++ source code generated on : Thu Apr  6 09:34:55 2017
//
...
#define catvehicle_hoff_MessageQueueLen (1)
...
void catvehicle_hoffmannFollower_step(void)
{
  ...
  varargout_1 = Sub_catvehicle_hoffmannFollower_13.getLatestMessage
    (&catvehicle_hoffmannFollower_B.varargout_2);
  ...
  // MATLAB Function: '<S5>/circle centered' incorporates:
  //   Constant: '<S5>/radius'

  // MATLAB Function 'Select desired path/circle centered': '<S14>:1'
  // '<S14>:1:3' angles=linspace(-pi,pi,301);
  // '<S14>:1:4' r = radius*exp(angles*1j);
  ...
  // MATLAB Function: '<S5>/straight line' incorporates:
  //   Constant: '<S5>/heading'
  //   Constant: '<S5>/len'

  // '<S14>:1:5' xs = real(r)-radius+start(1);
  // '<S14>:1:6' ys = imag(r)+start(2);
  // '<S14>:1:7' phis=(pi/2)+angles;
  ...
  // MATLAB Function 'Select desired path/straight line': '<S15>:1'
  // '<S15>:1:3' r = linspace(0,len,301);
  ...
  if ((catvehicle_hoffmannFollower_P.len_Value < 0.0) && (fabs
    ...
    delta1 = catvehicle_hoffmannFollower_P.len_Value / 300.0;
    for (b_index = 0; b_index < 299; b_index++) {
      catvehicle_hoffmannFollower_B.ys[1 + b_index] = (1.0 + (real_T)b_index) *
        delta1;
    }
  } else {
    delta1 = catvehicle_hoffmannFollower_P.len_Value / 300.0;
  ...
  // '<S15>:1:4' xs = r*cos(heading);
  delta1 = cos(catvehicle_hoffmannFollower_P.heading_Value);
  for (i = 0; i < 301; i++) {
    catvehicle_hoffmannFollower_B.xs[i] = catvehicle_hoffmannFollower_B.ys[i] *
      delta1;
  }

  // '<S15>:1:5' ys = r*sin(heading);
  delta1 = sin(catvehicle_hoffmannFollower_P.heading_Value);
  for (b_index = 0; b_index < 301; b_index++) {
    catvehicle_hoffmannFollower_B.ys[b_index] *= delta1;
  }

  // End of MATLAB Function: '<S5>/straight line'

  // MultiPortSwitch: '<S5>/Multiport Switch' incorporates:
  //   Constant: '<S5>/heading'
  //   Constant: '<S5>/pathChoice '
  //   Constant: '<S5>/radius'
  //   Constant: '<S5>/start point'
  //   MATLAB Function: '<S5>/circle centered'

  // '<S15>:1:6' phis = heading*ones(1,length(r));
  if ((int32_T)catvehicle_hoffmannFollower_P.pathChoice_Value == 1) {
    for (b_index = 0; b_index < 301; b_index++) {
  ...
  // MATLAB Function 'Hoffmann Controller/calculate e': '<S9>:1'
  // '<S9>:1:3' path=reshape(pathV,[],3);
  // '<S9>:1:4' x1 = path(1,1);
  // '<S9>:1:5' y1 = path(1,2);
  // '<S9>:1:6' phi1=path(1,3);
  // '<S9>:1:7' etmp = inf;
  // '<S9>:1:8' e = inf;
  delta1 = (rtInf);
  ...
  // '<S9>:1:11' for i=1:length(path)
  for (i = 0; i < 301; i++) {
    // '<S9>:1:12' etmp = sqrt((x-path(i,1))^2+(y-path(i,2))^2);
    rtb_Product3_n = catvehicle_hoffmannFollower_B.In1.Pose.Pose.Position.X -
      catvehicle_hoffmannFollower_B.MultiportSwitch[i];
    ...
    // '<S9>:1:13' if( etmp < e )
    if (rtb_Product3_n < delta1) {
      // '<S9>:1:14' e = etmp;
      delta1 = rtb_Product3_n;
  ...
  // '<S9>:1:19' xpathOut=path(index,1);
  // '<S9>:1:20' ypathOut=path(index,2);
  // '<S9>:1:21' phipath=path(index,3);
  // '<S9>:1:23' ex1 = (path(index,1)-x);
  // '<S9>:1:24' ey1 = (path(index,2)-y);
  // '<S9>:1:25' ex2 = cos(phipath);
  // '<S9>:1:26' ey2 = sin(phipath);
  // '<S9>:1:27' sinnnn = ex1*ey2-ey1*ex2;
  // '<S9>:1:28' e = sign(sinnnn)*e;
  rtb_Product3_n = (catvehicle_hoffmannFollower_B.MultiportSwitch[b_index] -
                    catvehicle_hoffmannFollower_B.In1.Pose.Pose.Position.X) *
    sin(catvehicle_hoffmannFollower_B.MultiportSwitch[602 + b_index]) -
    (catvehicle_hoffmannFollower_B.MultiportSwitch[301 + b_index] -
     catvehicle_hoffmannFollower_B.In1.Pose.Pose.Position.Y) * cos
    (catvehicle_hoffmannFollower_B.MultiportSwitch[602 + b_index]);
  ...
  // Product: '<S18>/Product'
  rtb_fcn5 = catvehicle_hoffmannFollower_B.In1.Pose.Pose.Orientation.W /
    rtb_Product3_n;

  // Product: '<S18>/Product1'
  rtb_Product1_p = catvehicle_hoffmannFollower_B.In1.Pose.Pose.Orientation.X /
    rtb_Product3_n;

  // Product: '<S18>/Product2'
  rtb_Product2_l = catvehicle_hoffmannFollower_B.In1.Pose.Pose.Orientation.Y /
    rtb_Product3_n;

  // Product: '<S18>/Product3'
  rtb_Product3_n = catvehicle_hoffmannFollower_B.In1.Pose.Pose.Orientation.Z /
    rtb_Product3_n;
  ...
  rtb_Product3_n = rt_atan2d_snf((rtb_Product1_p * rtb_Product2_l + rtb_fcn5 *
    rtb_Product3_n) * 2.0, ((rtb_fcn5 * rtb_fcn5 + rtb_Product1_p *
    rtb_Product1_p) - rtb_Product2_l * rtb_Product2_l) - rtb_Product3_n *
    rtb_Product3_n) - catvehicle_hoffmannFollower_B.MultiportSwitch[602 +
    b_index];
  ...
  // MATLAB Function 'Hoffmann Controller/calculate delta': '<S8>:1'
  // '<S8>:1:3' if( abs(v) < eps )
  ...
  if (rtb_Product3_n < -3.1415926535897931) {
    // '<S8>:1:10' phi = phi+2*pi;
    rtb_Product3_n += 6.2831853071795862;
  } else {
    if (rtb_Product3_n > 3.1415926535897931) {
      // '<S8>:1:11' elseif( phi > pi )
      // '<S8>:1:12' phi = phi-2*pi;
      rtb_Product3_n -= 6.2831853071795862;
    }
  }
  ...
  if (delta1 > catvehicle_hoffmannFollower_P.Switch_Threshold) {
    // BusAssignment: '<Root>/Bus Assignment' incorporates:
    //   Constant: '<S2>/k1'
  ...
  catvehicle_hoffmannFollower_B.BusAssignment.Angular.Z = (rt_atan2d_snf
    (catvehicle_hoffmannFollower_P.k_Value * delta1, rtb_fcn5) + rtb_Product3_n)
...
void catvehicle_hoffmannFollower_initialize(void)
{
...

  {
    ...
    tmp_2[16] = '\x00';
    Sub_catvehicle_hoffmannFollower_13.createSubscriber(tmp_2,
      catvehicle_hoff_MessageQueueLen);
    ...
    tmp_1[19] = '\x00';
    Pub_catvehicle_hoffmannFollower_9.createPublisher(tmp_1,
      catvehicle_hoff_MessageQueueLen);
...
void catvehicle_hoffmannFollower_terminate(void)
{
  ...
  if (catvehicle_hoffmannFollower_DW.obj_l.isInitialized == 1) {
    catvehicle_hoffmannFollower_DW.obj_l.isInitialized = 2;
  }
  ...
  if (catvehicle_hoffmannFollower_DW.obj.isInitialized == 1) {
    catvehicle_hoffmannFollower_DW.obj.isInitialized = 2;
  }
